      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-lg">
        <button
          ...
          className={`flex-1 py-1.5 text-xs font-medium rounded-md transition ${
            mode === 'preset' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500'
          }`}
        >
          🖼️ 预设图库
        </button>
        ...
        <div>
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-300 focus:border-orange-400"
              onKeyDown={e => e.key === 'Enter' && handleUrlSubmit()}
            />
            <button
              type="button"
              onClick={handleUrlSubmit}
              className="px-3 py-2 bg-primary text-white text-sm font-medium rounded-lg hover:bg-primary-dark transition"
            >
              确认
            </button>
          </div>
          <p className="text-xs text-gray-400 mt-1">支持 jpg/png/webp 格式的图片链接</p>
        </div>
      )}
        ...
        <div className="grid grid-cols-4 gap-2 max-h-48 overflow-y-auto">
          {presetCovers.map((img) => (
            <button
              ...
              className={`relative rounded-lg overflow-hidden aspect-[3/2] border-2 transition ${
                preview === img.url ? 'border-primary shadow-md' : 'border-transparent hover:border-gray-300'
              }`}
            >
